$(function () {
	...
	$generateBtn.click(function() {
		game.buttonsWorker([$growOldBtn, $explodeBtn, $slowUpBtn, $pauseBtn, $barrierBtn], 'activateBtn');
		game.deactivateBtn($autoGenerateBtn);
		game.generate();
	});
	
	$autoGenerateBtn.click(function() {
		autoGeneratedCount++;
...
game = {	
	...
	pauseGame: function() {
		if (isPlay == true) {
			isPlay = false;
			game.buttonsWorker([$growOldBtn, $explodeBtn, $slowUpBtn, $barrierBtn], 'deactivateBtn');
			$pauseBtn.text("Play");
			$pauseBtn.css({"border":"1px solid chartreuse"});
		}
		else {
			isPlay = true;
			game.buttonsWorker([$growOldBtn, $explodeBtn, $slowUpBtn, $barrierBtn], 'activateBtn');
			$pauseBtn.text("Pause");
			$pauseBtn.css("border", "none");
		}
	},
	
	growPlant: function() {
		var line = random(0, $lines.length);
		var position = random(260, 780);
		var plant = new Plant();
	...
	buttonsWorker: function(buttons, method) {
		for (var btn in buttons) {
			game[method](buttons[btn]);
		}
	}
